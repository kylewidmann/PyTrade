...
class MarketData(IDataContext):
    ...
    def get(self, instrument: Instrument, granularity: Granularity) -> IInstrumentData:
        # First try to find an exact match for the requested granularity
        exact = next(
        ...
        # Fall back to M1 source and resample up to the requested granularity
        m1_source = next(
            (src for src in self._sources
        ...
        _df = m1_source.df.resample(UPDATE_MAP[granularity]).agg(
            {
                "open": "first",
                "high": "max",
                "low": "min",
        ...
        return InstrumentData(instrument=instrument, granularity=granularity, df=_df)
